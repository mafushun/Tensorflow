...
with tf.Graph().as_default():
    ...
    with tf.name_scope('inference'):
        ...
        with tf.name_scope('Conv2d'):
            conv_kenerl_num=32
            stdd=0.001
            weights=WeightVariable(shape=[5,5,1,conv_kenerl_num],name_str='weights',stddev=stdd)
        ...
        with tf.name_scope('activity'):
            activaty_out=activity(conv_out,activity=tf.nn.relu6,name='relu6')
        #池化层
        ...
        with tf.name_scope('FC_Linear'):
            weights=WeightVariable([12*12*conv_kenerl_num,n_classes],name_str='weights',stddev=stdd)
    ...
    with tf.name_scope('Train'):
        ...
        optiminer=tf.train.FtrlOptimizer(learning_rate=learning_rate_train)
